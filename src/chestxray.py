...
class ChestXrayDataset(data.Dataset):
    def __init__(
        ...
        if mode == 'train':
            ...
            self.cls_names = obj_name if obj_name is not None else list(meta_info.keys())
            for cls_name in self.cls_names:
    ...
    def combine_img(self, cls_name):
        ...
        for i in range(4):
            ...
            mask_path = os.path.join(
                self.root, cls_name, 'ground_truth',
                random_defect, random_file[:3] + '_mask.png',
            )
        ...
        for i, img in enumerate(img_ls):
            result_image.paste(img, ((i % 2) * image_width, (i // 2) * image_height))

        result_mask = Image.new("L", (2 * image_width, 2 * image_height))
        for i, img in enumerate(mask_ls):
            result_mask.paste(img, ((i % 2) * image_width, (i // 2) * image_height))
